...
#### Forecasts over 10 days: https://github.com/ripa1993/DM-Project/blob/master/Results/PREDIZIONE_FINALE_FUTURE.csv
...
```{r echo = FALSE, fig.align="center"}
  ...
  selected <- filter(dataset_polimi, sottoarea == 20, prod == "1")

  tsdisplay(selected$vendite, main = "Time-series, acf and parcor of subarea 20, product 1")
...
```{r echo=F, warning=F, tidy = T, comment = "#", collapse=T, fig.align="center"}
    ...
    map <- getMap(resolution = "low")
    plot(map, xlim = c(5, 20), ylim = c(36, 47), asp = 1, main = "6 Clusters")
    for (i in unique(dataset_polimi[, "cluster6"])) {
...
plot <- ggplot(rfs_importance_df, aes(x = reorder(name, rfs_importance),
       ...
       coord_flip() +
       theme_minimal() + ggtitle("Importance plot for a small portion of the dataset")
print(plot)
``` 
...
```{r echo=F, tidy = T, comment = "#", collapse=F}
  train <- filter(train, prod == 1, sottoarea == 5)
  test <- filter(test, prod == 1, sottoarea == 5)
  pred_forest <- filter(pred_forest, prod == 1, sottoarea == 5)
  pred_sarima <- filter(pred_sarima, prod == 1, sottoarea == 5)
  pred_xgboost <- filter(pred_xgboost, prod == 1, sottoarea == 5)
  pred_ensemble <- filter(pred_ensemble, prod == 1, sottoarea == 5)

  cols <- c("train" = "#000000", "test"="#604020","xgboost"="#e65c00","forest"="#009933", "sarima" = "#e6005c", "ensemble" = "#0059b3")

  p <- ggplot(train, aes(x = data, y = vendite, col = "train")) + geom_line(size = 1.2, alpha = 0.8) + geom_point(size=4) + geom_line(data=test, aes(x=data, y = vendite, col = "test"),size = 1.2, alpha = 0.8) + geom_point(data=test, aes(x=data, y = vendite, col = "test"),size = 3) + ggtitle("Predictions on the test set for subarea 5, product 1 - 10 days") + theme_minimal() + xlab("Data") + ylab("Numero di vendite")
