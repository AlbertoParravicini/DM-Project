...
```{r echo=F, tidy = T, comment = "#", collapse=F}
  ...
  p <- ggplot(train, aes(x = data, y = vendite, col = "train")) + geom_line(size = 1.2) + geom_point(size=3) + geom_line(data=test, aes(x=data, y = vendite, col = "test"),size = 1.2) + geom_point(data=test, aes(x=data, y = vendite, col = "test"),size = 3) + ggtitle("Predictions on the test set, over 10 days") + theme_minimal() + xlab("Data") + ylab("Numero di vendite")

  if (input$sarima) {
    p <- p + geom_line(data=pred_sarima, aes(x=data, y = vendite, col = "sarima"),size = 1.2) + geom_point(size = 3,data=pred_sarima, aes(x=data, y = vendite, col = "sarima"))
  }
  if (input$xgboost) {
    p <- p + geom_line(data=pred_xgboost, aes(x=data, y = vendite, col = "xgboost"),size = 1.2) + geom_point(size = 3,data=pred_xgboost, aes(x=data, y = vendite, col = "xgboost"))
  }
  if (input$forest) {
    p <- p + geom_line(data=pred_forest, aes(x=data, y = vendite, col = "forest"),size = 1.2) + geom_point(size = 3,data=pred_forest, aes(x=data, y = vendite, col = "forest"))
  }
  if (input$ensemble) {
    p <- p + geom_line(data=pred_ensemble, aes(x=data, y = vendite, col = "ensemble"),size = 1.2) + geom_point(size = 3,data=pred_ensemble, aes(x=data, y = vendite, col = "ensemble"))
  }
