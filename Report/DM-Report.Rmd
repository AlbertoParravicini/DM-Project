...
```{r echo=F}
 gps <- read.csv("../gps.csv", stringsAsFactors = F)
...
```
```{r echo=F, warning=F, tidy = T, comment = "#", collapse=T}
   selectInput("cluster", label = "Select a number of clusters:",
               choices = c("3 clusters", "6 clusters", "20 clusters"), selected = "3 clusters")
...
After various trials, by making use of metrics such as the *mean squared error*, the *AIC*[^1], and the estimated variance of the parameters, we opted for a **(1, 1, 1)(1, 1, 1)_7** model.  
However, the analysis of the residuals showed some traces of a remaining seasonality, of different latency. We then added a second *Seasonal ARIMA*, of order **(0, 0, 0)(1, 1, 1)_6**, to model the residual of the first model, and a third *Seasonal ARIMA*, of order **(0, 0, 0)(1, 1, 0)_5**, to fit the residuals of the second.  
The overall prediction is the sum of the predictions of the three models.
The residuals of this stacked *Seasonal ARIMA* can be considered white noise (low p-value: very likely to be white noise).
