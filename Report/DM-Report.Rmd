...
renderPlot({
  ...
  tsdisplay(selected$vendite, main = "Analysis of the selected time-series")
})
```
...
```{r echo = FALSE}
dataset <- read.csv("~/DM-Project/Modified data/dataset_polimi_final_with_holidays_v2.csv", stringsAsFactors=FALSE)
...
pred_xgboost <- read.csv("~/DM-Project/Results/predizione_xgboost_TEST.csv", row.names=NULL, stringsAsFactors=FALSE)
pred_forest <- read.csv("~/DM-Project/Results/predizione_random_forest_TEST.csv", row.names=NULL, stringsAsFactors=FALSE)
pred_sarima <- read.csv("~/DM-Project/Results/predizione_sarima_TEST.csv", row.names=NULL, stringsAsFactors=FALSE)
pred_ensemble <- read.csv("~/DM-Project/Results/PREDIZIONE_CON_PESI_TEST.csv", row.names=NULL, stringsAsFactors=FALSE)
...
```{r echo=F, tidy = T, comment = "#", collapse=F}
  ...
  cols <- c("train" = "#000000", "test"="#f04546","xgboost"="#3591d1","forest"="#62c76b", "sarima" = "#123abc", "ensemble" = "#ccc333")

  p <- ggplot(train, aes(x = data, y = vendite, col = "train")) + geom_line(size = 1) + geom_point(size=2) + geom_line(data=test, aes(x=data, y = vendite, col = "test"),size = 1) + geom_point(data=test, aes(x=data, y = vendite, col = "test"),size = 2) + ggtitle("Predictions on the test set, over 10 days") + theme_minimal() + xlab("Data") + ylab("Numero di vendite")

  if (input$sarima) {
    p <- p + geom_line(data=pred_sarima, aes(x=data, y = vendite, col = "sarima"),size = 1) + geom_point(size = 2,data=pred_sarima, aes(x=data, y = vendite, col = "sarima"))
  }
  if (input$xgboost) {
    p <- p + geom_line(data=pred_xgboost, aes(x=data, y = vendite, col = "xgboost"),size = 1) + geom_point(size = 2,data=pred_xgboost, aes(x=data, y = vendite, col = "xgboost"))
  }
  if (input$forest) {
    p <- p + geom_line(data=pred_forest, aes(x=data, y = vendite, col = "forest"),size = 1) + geom_point(size = 2,data=pred_forest, aes(x=data, y = vendite, col = "forest"))
  }
  if (input$ensemble) {
    p <- p + geom_line(data=pred_ensemble, aes(x=data, y = vendite, col = "ensemble"),size = 1) + geom_point(size = 2,data=pred_ensemble, aes(x=data, y = vendite, col = "ensemble"))
  }
