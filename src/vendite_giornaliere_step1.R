...
dataset <- dataset[ , !(names(dataset) %in% c("vendite_giorn_prod.x", "vendite_giorn_tot.x", "azienda_chiusa.y"))]
colnames(dataset)[which(colnames(dataset) == 'vendite_giorn_prod.y')] <- 'vendite_giorn_prod'
colnames(dataset)[which(colnames(dataset) == 'vendite_giorn_tot.y')] <- 'vendite_giorn_tot'
colnames(dataset)[which(colnames(dataset) == 'azienda_chiusa.x')] <- 'vendite_missing'

dataset <- dataset[order(dataset$prod, dataset$zona, dataset$area, dataset$sottoarea, dataset$data), ]
...
write.csv(dataset, file="Modified data/dataset_polimi_final_with_holidays_v2.csv", row.names = F)
